...
const OrganizingCommitteCard = ({title,image}) => {
  return (
    <div className="relative group transition-all duration-300 h-24 w-[320px] bg-gray-950 text-gray-300 rounded-3xl overflow-hidden my-2" >
        <div className='absolute left-10 group-hover:text-center group-hover:left-28 transition-all duration-300' >
            <h1 className='text-4xl font-medium' ><span className='font-light text-sm px-1 group-hover:bg-teal-400 group-hover:text-black group-hover:cursor-pointer group-hover:font-medium group-hover:p-1 group-hover:rounded-md ' >Explore</span><br /> {title}</h1>
        </div>
